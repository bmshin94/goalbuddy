...
Direct verification commands must launch Node or an exact local executable. A Node file operand must name an **exact existing local file**, including an extensionless file, inside the authorized workspace. Directory launches (`node .`, `node checks`, trailing-slash variants) and unresolved file paths reject before execution, even with package `main`/`index.js`, colliding sibling files or explicit inputs. No directory, extension-search or main fallback is inferred. Recover by naming the concrete validator, for example `node checks/run.mjs`, and declaring additional dependencies in `acceptance.inputs`. Optional `--no-warnings`, `--trace-warnings` or `--` may precede the file. Supported inline spellings are `-e <literal code>`, `--eval <literal code>`, `--eval=<literal code>`, `-p <literal code>` and `--print <literal code>`; exact argv/metadata binds the code, with additional dependencies declared in `acceptance.inputs`. Separate inline options require a literal code operand; another option in that position is an uncertain launch and rejects. The ambiguous Node invocation option `--print=<...>` rejects before execution, including in package scripts. Recover with separate `--print <literal code>` or `--eval <literal code>` for inline code, or `node checks/run.mjs` for a real validator file. `--print=` text after a real script operand or passed to a non-Node validator remains ordinary argument data. For inline checks, use `--` before option-like argument data; unresolved `--print=` words among remaining option/operand pairs reject. Other unsupported Node options and opaque external wrappers reject.

On Linux/macOS, supported npm/pnpm/yarn run/lifecycle launches bind package metadata/lockfiles and inspect the selected script and its pre/post hooks as one literal Node command or exact local executable. Only unquoted ASCII space/tab separate words; Unicode whitespace stays part of the filename. Single/double quotes and escapes are supported; CR/LF, shell expansions, composition, assignments and nested launcher/shell wrappers reject. Logging flags such as `npm run --silent accept` are resolved before selecting the script; `--` ends launcher-option parsing. Unknown flags, workspace-changing options, ambiguous commands and implicit/missing scripts reject. Use a direct command naming the concrete validator for unsupported forms.

On native Windows, direct Node argv stays shell-free, including literal spaces, Unicode and shell punctuation in file operands. Native `.exe`/`.com` local executables are supported; batch/PowerShell/POSIX scripts require a direct concrete Node validator instead. `npm`/`npm.cmd` run/lifecycle commands use the current Node installation's bundled `node_modules/npm/bin/npm-cli.js` with literal argv. The recorder pins the workspace and explicitly selects native `System32/cmd.exe` via npm's `--script-shell` option, overriding npm configuration; the proof binds that launch argv, npm entry hash and shell identity. Other package managers, custom npm shims and missing bundled npm reject with direct Node recovery. Earlier Windows npm attempts without this launch binding require fresh observed acceptance; preserve their files.

Windows npm scripts support unquoted `node`/`node.exe` or an unquoted path to the current `node.exe`, followed by literal words or whole double-quoted words. Backslashes are literal path separators, not POSIX escapes; only ASCII space/tab separate words and Unicode remains literal. Other native validators use direct argv. Shell builtins, command prefixes, single quotes, escaped/concatenated quotes, backslashes before closing quotes, expansions, chaining and shell metacharacters reject before execution. Quoted executable paths also reject because npm/cmd command-string quote removal can differ from Node argv; use direct Node argv for those paths. Logging flags remain supported; launcher/script arguments supplied in the acceptance argv must be simple ASCII option/script words. The selected script and pre/post hooks remain bound. Native executable lookup includes npm's ancestor `.bin` paths and rejects a shadow/different Node runtime. Directory/main/index inference remains unsupported. Recover with argv such as `["C:\\Program Files\\nodejs\\node.exe", "checks/run.mjs"]`, explicitly declaring additional dependencies. Existing POSIX quoting behavior is unchanged.

The exact-file and `--print=` restrictions deliberately narrow the unshipped 0.5.0 candidate. Earlier directory-based or `--print=` invocation attempts cannot certify completion under this checker. Preserve those receipts and attempts; authorize a fresh final audit using a supported command and consume its new observed proof as described below.
